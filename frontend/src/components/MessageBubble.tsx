    ...
    <div className={`ui-static flex group ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        <div
          ...
          style={{
            background: isError
              ? 'var(--composer-danger-bg)'
              : isUser
                ? 'var(--bg-user-bubble)'
                : 'var(--bg-asst-bubble)',
            color: isError
              ? 'var(--composer-danger-text)'
              : isUser
                ? 'var(--text-user-bubble)'
                : 'var(--text-asst-bubble)',
            border: isError ? '1px solid var(--composer-danger-border)' : 'none',
          }}
        >
          {isError ? (
        ...
        {!message.isStreaming && displayContent && (
          <button
            ...
            className="flex items-center gap-1 rounded px-2 py-0.5 text-xs opacity-0 transition-opacity group-hover:opacity-100"
            style={{
