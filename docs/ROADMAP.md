...
|-------|--------|---------|
| **Section 13.0** | Done | Numbered SQL migrations + `schema_migrations` checksums; stable JSON errors `{ detail, code, request_id }` + handlers; `X-Request-ID` honored. |
...
| Order | Focus | Notes |
|-------|--------|--------|
| **0** | **Section 14.7 RAG quality closure** | CI/regression eval, thresholds, golden-set process, light observability - see [Section 14.7](#147-rag-quality-closure-post-146--in-progress--next). |
...
| **v1.5.x** | Phase **14** (through **14.6**) and **Vision MVP** are **ship-complete on main**; active follow-up is **Section 14.7** (RAG quality closure), then **Phase 15**. |
| **v1.6+** | **Phase 15** - semantics and structural overhaul, optional normalization & tracing (**consumes** Phase 13 Section 13.0 error model; **does not redefine** it). May overlap late **v1.5.x** once **Section 14.7** exit criteria are clear. |
...
### 3. RAG quality - protocols shipped; Section 14.7 closure landed

| Reality (main) | Improvement path | Roadmap / docs home |
|----------------|------------------|---------------------|
| **14.6** delivered rerank/rewrite **Protocols**, `retrieval_profile`, `tools/run_rag_eval.py`, `evaldata/`. | **Section 14.7:** CI runs `run_rag_eval.py` every backend build; OPERATIONS documents knobs; `evaldata/README.md` + `VERSION`; metrics on profile/outcome and rewrite. | **Section 14.7** below; [README.md](../README.md) / [PROJECT_STATUS.md](PROJECT_STATUS.md) "RAG-ready" wording. |
| Further tuning (score cutoffs, dashboards) | Optional; not blocking Section 14.7. | Phase **15.6** optional. |

**Priority:** **Section 14.7** is complete; iterate eval cases as retrieval behavior evolves.
...
**Priority:** **High** leverage - lowers cost of Section 14.7 and Phase **15.1** invariant tests.
...
|-------|-------------------------|
| Shared key + future minimal AuthZ | Section 15.5, Section 15.1, SECURITY / OPERATIONS |
| Single-instance SQLite + local vector | Appendix, OPERATIONS, Phase 13 risk triggers; future datastore = Decision Log |
| RAG eval + thresholds + CI | **Section 14.7** (complete - CI + OPERATIONS + evaldata + metrics) |
| Integration smoke + clock (idempotency) | Section 15.3 (initial); broader injection follow-up |
| DOMAIN.md + policy + invariants | Section 15.1-15.2 |
...
**Priority order (remaining):** **Section 14.7** (RAG quality closure), then **Phase 15**.
...
| **Thresholds** | `[x]` | RAG knobs and no-hit behavior documented in [OPERATIONS.md](OPERATIONS.md) **RAG retrieval quality**; [ENGINEERING_STANDARDS.md](ENGINEERING_STANDARDS.md) Section 12 points to OPERATIONS + evaldata. |
...
**Dependencies:** Section 15.3 integration fixtures are available; use them when tightening retrieval beyond pure mocks.
...
**Goal:** **Semantic convergence first, directory migration second**-policies and invariants stabilize meaning before `application/` vs `domain/` vs `infra/` reshaping. Phase 15 **consumes** the Phase 13 Section 13.0 **error model** (stable `code`, `request_id`, handlers); it **does not redefine** that contract-only uses it for policies, tests, and optional tracing.
...
**Ordering principle (industrial default):** policy objects + invariants = **narrow, testable moves**; package reshuffle = **wide blast radius**. Do the former first. **Section 14.7** is complete; proceed with Phase **15** when ready-large AuthZ or datastore changes remain **Decision Log** items.
...
### 15.2 Large structural migration (after Section 15.1) - **initial slice complete**
...
- `[x]` **Ports list** -[PORTS.md](PORTS.md) + link from [ENGINEERING_STANDARDS.md](ENGINEERING_STANDARDS.md) Section 1.9; lists `SessionRepository`, `ConversationLogger`, `TitleGenerator`, `LLMClient`, `Settings`, `SafeguardService`, `TabularContextExtractor`, and stable shared exceptions so routers/application import from one contract face.
...
- `[x]` **Single primary test entry** -README + [ENGINEERING_STANDARDS.md](ENGINEERING_STANDARDS.md) Section 3.1: **`pytest`** canonical; `unittest discover` legacy/optional. CI unchanged (`python -m pytest __tests__/`).
...
| 2026-04-08 | Docs: multi-environment portability | README/OPERATIONS/ENGINEERING_STANDARDS clarify the repo is not limited to one school host; optional high-risk features (e.g. code sandbox) are policy-gated per **ENGINEERING_STANDARDS Section 15**. |
| 2026-04-08 | ROADMAP: current reality map + Section 14.7 | Added **Current reality and improvement map** (shared API key, SQLite/local vector, RAG loop, tests, domain semantics, shared-host process model) with **Summary** table; added **Section 14.7 RAG quality closure** (CI eval, thresholds, golden-set process, observability); **Next** program order is **0 = Section 14.7**, **1 = Phase 15**. |
