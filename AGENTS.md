...
- Keep exact dependency locks:
  - `requirements.txt` pinned
  - `package-lock.json` committed
  - use `npm ci`, not `npm install`, in CI/deploy
...
- `systemctl --user` may work, but may also fail in SSH sessions. Always preserve the `nohup` + `fastapi.pid` fallback path.
...
- Use layered error handling:
  - services/clients raise domain exceptions
  - route handlers translate them to sanitized `HTTPException`
  - startup config errors should stop the app immediately
...
- Python tests are commonly run with `python -m unittest discover -s __tests__ -p "test_*.py" -v`
- Some historical notes mention pytest style guidance; preserve isolation/mocking principles regardless of runner
...
This memory file was derived from:

- `.cursor/rules/core-principles.mdc`
- `.cursor/rules/server-environment.mdc`
