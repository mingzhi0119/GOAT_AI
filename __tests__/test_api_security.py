...
class InMemoryRateLimiterTests(unittest.TestCase):
    def test_allow_within_threshold(self) -> None:
        ...
        allowed1, retry1 = limiter.allow("key", now=100.0, window_sec=60, max_requests=2)
        allowed2, retry2 = limiter.allow("key", now=101.0, window_sec=60, max_requests=2)
    ...
    def test_blocks_after_threshold_and_returns_retry_after(self) -> None:
        ...
        allowed, retry_after = limiter.allow("key", now=102.0, window_sec=60, max_requests=2)
    ...
    def test_expires_old_requests_after_window(self) -> None:
        ...
        blocked, retry_after = limiter.allow("key", now=120.0, window_sec=60, max_requests=1)
        self.assertFalse(blocked)
        self.assertEqual(40, retry_after)

        allowed_again, retry_again = limiter.allow("key", now=161.0, window_sec=60, max_requests=1)
    ...
    def test_tracks_each_key_independently(self) -> None:
        ...
        blocked_a, retry_a = limiter.allow("key-a", now=101.0, window_sec=60, max_requests=1)
        allowed_b, retry_b = limiter.allow("key-b", now=101.0, window_sec=60, max_requests=1)
