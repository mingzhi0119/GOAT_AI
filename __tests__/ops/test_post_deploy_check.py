...
from tools.ops.post_deploy_check import (
...
def test_expect_runtime_target_accepts_server_port() -> None:
    ...
    with patch("tools.ops.post_deploy_check.requests.get", return_value=response):
...
def test_expect_chat_stream_contract_accepts_thinking_only_stream() -> None:
    ...
    with (
        patch("tools.ops.post_deploy_check.requests.get", return_value=model_response),
        patch("tools.ops.post_deploy_check.requests.post", return_value=chat_response),
    ):
...
def test_expect_chat_stream_contract_accepts_token_then_done() -> None:
    ...
    with (
        patch("tools.ops.post_deploy_check.requests.get", return_value=model_response),
        patch(
            "tools.ops.post_deploy_check.requests.post", return_value=chat_response
        ) as post,
    ):
        assert (
            _expect_chat_stream_contract(
