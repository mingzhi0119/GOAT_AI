...
class CodeSandboxRuntimeTests(unittest.TestCase):
    ...
    def test_repository_marks_running_execution_cancelled_with_outputs(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            db_path = Path(tmp) / "sandbox.db"
            apply_migrations(db_path)
            repository = SQLiteCodeSandboxExecutionRepository(db_path)
            ...
            repository.create_execution(
                CodeSandboxExecutionCreatePayload(
                    execution_id="cs-running-cancelled",
                    execution_mode="async",
                    runtime_preset="shell",
                    network_policy="disabled",
                    timeout_sec=8,
            ...
            repository.mark_execution_cancelled(
                ...
                output_files=[{"path": "partial.txt", "byte_size": 7}],
            )

            record = repository.get_execution("cs-running-cancelled")
            assert record is not None
            self.assertEqual("cancelled", record.status)
            self.assertEqual("Execution cancelled by request.", record.error_detail)
            self.assertEqual(
                [{"path": "partial.txt", "byte_size": 7}],
                record.output_files,
            )

            events = repository.list_execution_events("cs-running-cancelled")
            self.assertEqual(
                ["execution.queued", "execution.started", "execution.cancelled"],
                [event.event_type for event in events],
            )
