...
class RuntimeTargetTests(unittest.TestCase):
    def test_auto_prefers_server_then_local_when_server_port_is_bindable(self) -> None:
        ...
        self.assertEqual([item.port for item in targets], [62606, 8002])
        self.assertEqual(targets[0].mode, "server62606")
        self.assertEqual(targets[1].mode, "local")

    def test_auto_falls_back_to_local_when_server_port_is_unavailable(self) -> None:
        settings = make_settings()
        ...
        self.assertEqual([item.port for item in targets], [8002])
        self.assertEqual(targets[0].mode, "local")
    ...
    def test_explicit_local_mode_returns_only_local_port(self) -> None:
    ...
    def test_current_runtime_target_describes_local_port(self) -> None:
        settings = make_settings()

        target = current_runtime_target(settings, current_port=8002)

        self.assertEqual(target.mode, "local")
        self.assertEqual(target.base_url, "http://127.0.0.1:8002")
