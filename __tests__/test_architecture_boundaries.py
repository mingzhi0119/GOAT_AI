...
class ArchitectureBoundaryTests(unittest.TestCase):
    def test_backend_services_do_not_import_fastapi(self) -> None:
    ...
    def test_backend_routers_avoid_direct_infra_libraries(self) -> None:
        ...
        self.assertFalse(
            offenders,
            f"backend/routers should not directly import infrastructure libs: {offenders}",
        )

    def test_backend_services_confine_sqlite_log_imports(self) -> None:
        """SQLite persistence helpers stay in log_service + chat_runtime adapters only."""
        services_dir = REPO_ROOT / "backend" / "services"
        allowed = {
            services_dir / "log_service.py",
            services_dir / "chat_runtime.py",
        }
        offenders: list[str] = []
        for path in _python_files(services_dir):
            if path in allowed:
                continue
            text = _read_text(path)
            if "log_service" in text and (
                "from backend.services import log_service" in text
                or "from backend.services.log_service" in text
            ):
                offenders.append(str(path.relative_to(REPO_ROOT)))

        self.assertFalse(
            offenders,
            "backend/services must not import log_service except in log_service.py and "
            f"chat_runtime.py adapters: {offenders}",
        )

    def test_frontend_hooks_do_not_import_components(self) -> None:
