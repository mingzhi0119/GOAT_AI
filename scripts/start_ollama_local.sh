...
if curl -sf "${OLLAMA_HOST_VALUE}/api/tags" >/dev/null 2>&1; then
  echo "✅ Local Ollama already responding at ${OLLAMA_HOST_VALUE}"
  exit 0
fi
...
if [ ! -f "${PID_FILE}" ]; then
  echo "🚀 Starting local Ollama at ${OLLAMA_HOST_VALUE}..."
  nohup bash "${REPO_ROOT}/scripts/ollama_local.sh" serve >>"${LOG_FILE}" 2>&1 &
  echo $! > "${PID_FILE}"
fi

for _ in $(seq 1 30); do
  if curl -sf "${OLLAMA_HOST_VALUE}/api/tags" >/dev/null 2>&1; then
    echo "✅ Local Ollama is ready at ${OLLAMA_HOST_VALUE}"
    exit 0
  fi
  sleep 1
done
